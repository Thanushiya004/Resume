...
def extract_skills_from_text(spacy_nlp, matcher, text):
    ...
    if SPACY_AVAILABLE and spacy_nlp and matcher:
        import spacy
        try:
            doc = spacy_nlp(text)
        except:
        ...
        # Dummy document if spaCy fails to load
        class DummyToken:
            def __init__(self, t):
                self.text = t
        ...
        class DummyDoc:
            def __init__(self, txt):
                self.text = txt
                self.ents = []
                self.tokens = [DummyToken(w) for w in txt.split()]
            def __iter__(self):
    ...
    for sk in SKILL_PHRASES:
        if sk.lower() in token_counts or sk.lower() in text.lower():
